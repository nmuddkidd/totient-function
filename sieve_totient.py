...
def totient(n, primes):
    ...
    return int(result*n)  #<---------------THE ERROR IS RIGHT HERE!!! MAKE ONE SMALL CHANGE TO FIX THE ERROR.  
...
temp=len(primeArray)
if(totientValue==n):
    totientValue=totientValue-1
    temp=temp+1
print("There are {0!s} prime numbers less than or equal to {1!s}".format(temp,n)) #print the number of primes less than n
